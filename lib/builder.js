...
class Builder {
  ...
  */
  ...
  table(level, metadata, content) {
    return this.close().table(level, metadata, content);
  }
...
class CompoundBuilder extends Builder {
  ...
  table(level, metadata, content) {
    return new TableBuilder(this, level, metadata, content);
  }
...
class TableBuilder extends Builder {
  constructor(parent, level, metadata, content) {
    super(parent);
  ...
  table(level, metadata, content) {
    /*
      Tables can't be nested per Docbook 5.
      The table delimiter is implicitly an end delimiter.

      XXX Should we check `level` and allow nested tables?
    */
    return this.close();
  }

  close() {
    const node = new dom.Table(this.metadata, this.content);
