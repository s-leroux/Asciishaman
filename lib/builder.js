...
class Builder {
  ...
  blockDelimiter(level, delim) {
    this.close();
    return this.parent.blockDelimiter(level, delim);
  }
}
...
class DocumentBuilder extends Builder {
  ...
  blockDelimiter(level, delim) {
    return new BlockBuilder(this, this.container, level, delim);
  }
}
...
class BlockBuilder extends Builder {
  constructor(parent, container, level, delim) {
    super(parent, container);

    this.level = level;
    this.delim = delim;
    this.node = new dom.Block();
    this.container.children.push(this.node);
  }
