...
const WORD = grammar.define("word",
  peg.oneOrMore(peg.not(SP), peg.not("\n"), peg.any()),
  (...content) => content.join("")
);
const EMAIL = grammar.define("email",
  [ peg.consume("<"), peg.oneOrMore(peg.not(peg.charset(" \t\n>")), peg.any()), peg.consume(">") ],
  (...content) => content.join("")
);
...
grammar.define("author",
  [
    ...
    BLANK, 
    peg.zeroOrOne(
      peg.string(peg.not(SP), peg.not("\n"), peg.not(";"), peg.any()), BLANK, peg.not("<")
    ), 
    peg.string(peg.not(SP), peg.not("\n"), peg.not(";"), peg.any()),
