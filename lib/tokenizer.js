  ...
  */
  function stateDocumentFlow(c) {
    if (c === '\n') {
      ...
      _state = statePossiblyBlankLine;
    }
  }

  /*
    A '\n' was encountered. Three possible outcomes:
    - emit(NEW_LINE) if we find more content on the new line
    - emit(BLANK_LINE) if the new line is entirely blank
    - <noop> if we reach the end of the document

    The last case is a desirable side effect of emitting the
    NEW_LINE or BLANK_LINE token only when a non-blank and
    non-newline character is found.
  */
  function stateNewLine(c) {
    if ((c === ' ') || (c === '\t')) {
      _state = stateNewLine; // still can't decide
      _end = ++_pos;
    }
    else if (c === '\n') {
      ++_lineNo;
      _state = stateBlankLine; // collapse all blank lines
      _end = ++_pos;
    }
    else {
      emit(Tk.NEW_LINE);
  ...
  function stateBlankLine(c) {
    if ((c === ' ') || (c === '\t')) {
      _state = stateBlankLine; // defer emit(BLANK_LINE) until we find content
      _end = ++_pos;
    }
    else if (c === '\n') {
      ++_lineNo;
  ...
  /**
    Possibly-blank line
  */
  function statePossiblyBlankLine(c) {
    if ((c === ' ') || (c === '\t')) {
      _state = statePossiblyBlankLine; // still can't decide
      ++_pos;
    }
    else if (c === '\n') {
