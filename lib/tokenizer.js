...
// Tables
// ------------------------------------------------------------------------
grammar.define("table",
  [
    peg.optional(peg.rule("block-metadata"), []),
    peg.capture("|==", peg.oneOrMore("=")), EOL,
    peg.rule("table-content"),
    peg.capture("|==", peg.oneOrMore("=")), EOL
  ],
  function(metadata, begin, content, end) {
    //console.dir(content, {depth: Infinity});
    this.emit(Tk.TABLE, begin.length-1, metadata, content);
  }
);

grammar.define("table-content",
  peg.zeroOrMore(
    peg.choice(
      peg.rule("table-inline-row")
    ),
  )
);

grammar.define("table-inline-row",
  [
    peg.oneOrMore(
      peg.rule("table-inline-cell"),
    ),
    peg.zeroOrMore(EOL)
  ],
  (...cells) => { /* console.log("Row:", cells); */ return cells }
);

grammar.define("table-inline-cell",
  [
    peg.not("|==="),
    peg.consume("|", peg.zeroOrMore(SP)),
    peg.oneOrMore(
      peg.zeroOrMore(SP),
      peg.not("|"), peg.not(EOL,EOL), peg.not(EOL, "|"), peg.any()
    ),
    peg.consume(peg.zeroOrMore(SP), peg.zeroOrOne(EOL)),
  ],
  function(...content) {
    // console.log("Cell: ", content);
    return ip.parseText(content.join(""));
  }
);
