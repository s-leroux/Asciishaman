...
/*
  Warning:
  ========
  The tokenier uses a PEG. But due to the way later procesing
  stages are implemented, it _must_ not backtrack once a token
  has been emitted.
*/
...
grammar.define("example-block-delim",
  [ peg.capture(peg.oneOrMore("=")), peg.consume(peg.zeroOrMore(SP), "\n") ],
  function(delim) {
    return delim;
  }
);
...
grammar.define("section",
  [
    ...
    peg.rule("text"),
    peg.consume(peg.zeroOrMore(peg.zeroOrMore(SP), "\n"))
  ],
  function(level, text) {
